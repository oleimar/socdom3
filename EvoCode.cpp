    ...
    num_thrds{1}
{
    ...
    // Create N male "placeholder individuals" in population (based on the
...
#pragma omp parallel for num_threads(num_thrds)
        for (unsigned k = 0; k < G; ++k) {
            ...
            // distribution needed
            rand_uni uni(0, 1);
            // thread-local container for local group individuals (males)
            vind_type lmpop;
            lmpop.reserve(Nl);
            ...
            vph_type lgph;
            lgph.reserve(Nl);
            // vector of indices in lmpop of individuals in container lgph
            ui_type lgi;
            lgi.reserve(Nl);
            for (unsigned i = 0; i < Nl; ++i) {
        ...
        // survival from mating season and reproduction to the next year; run
        // through m_pop; check survival
        rand_uni uni(0, 1);
...
void Evo::SelectReproduce(mut_rec_type& mr)
{
    ...
    flt tot_wei = 0;
    for (unsigned i = 0; i < N; ++i) {
        const phen_type& ph = m_pop[i].phenotype;
        wei[i] = ph.ares;
        tot_wei += wei[i];
    }
    if (tot_wei > 0) {
        ...
        for (unsigned j = 0; j < N1; ++j) {
            ...
            ind_type& patind = m_pop[ipat];
            // find mother genotype for individual to be constructed
            unsigned imat = dsf(mr.eng);
            gen_type& matind = f_pop[imat];
            // construct offspring genotype
            gen_type offspr(patind.GetGamete(mr), matind.GetGamete(mr));
...
void Evo::UpdateAgeStr(unsigned yr1, rand_eng& eng)
{
    ...
    for (unsigned i = 0; i < N; ++i) {
        ...
        if (ph.age >= M) {
            ...
            // do healing, update fighting ability, and apply forgetting for
            // alive individuals
            if (ph.alive) {
                ph.dmg *= 1 - eta1;
    ...
    // then copy male offspring genotypes from m_offspr into the positions of
    ...
    // then copy female offspring into the positions corresponding to the
    // youngest age
...
void Evo::h5_read_pop(const std::string& infilename)
{
    ...
    for (unsigned i = 0; i < N; ++i) {
        m_pop[i].phenotype.ndom = uival[i];
    }
    // nOffspr
